...
namespace Genies.Sdk.Samples.AvatarStarter
{
    public class LoadMyAvatar : MonoBehaviour
    {
        ...
        private async void OnAvatarEditorOpened()
        {
            if (geniesLoginUI != null)
            {
            ...
            // await AvatarSdk.SetEditorSaveLocallyAndExitAsync("Test-Profile-ID");
            // string resourcePath = "Test-Profile-ID";
            // ScriptableObject profileData = Resources.Load<ScriptableObject>(resourcePath);
            // if (profileData != null)
            // {
            //     string json = JsonUtility.ToJson(profileData, true); // 'true' makes it pretty-printed
                
            //     Debug.Log("Loaded JSON: " + json);
            // }
            // var avatar = AvatarSdk.GetAvatarEditorAvatar();
            // var definition = avatar.SetDefinitionAsync
        }
        ...
        public ManagedAvatar GetLoadedAvatar()
        {
            return LoadedAvatar;
        }
